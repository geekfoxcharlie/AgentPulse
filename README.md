...
For TypeSafe Jev structured decisions, make a `TYPESAFE_API_KEY` available to the current process, then register the template. Jev is a System One model: send a `state` plus typed `choice`, `score`, and `noul` questions to `POST /v1/systemone` and get typed answers back, not generated text. The default model is `jev-latest` (currently `jev-1.13.0`). The health probe calls the non-generative `GET /v1/models`, which validates the key without evaluating any state or incurring inference; only the `/v1/systemone` evaluation call is billable:

```bash
agentpulse api add --template typesafe --configured-at ~/.zshenv
